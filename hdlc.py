...
class Frame(object):
    ...
    def _checkCRC(self):
        ...
        if not res:
            c1 = bytes(crc)
            c2 = bytes(calcCRC(data_without_crc))
            logger.warning("invalid crc %s != %s <- our calculation", c1.hex(), c2.hex())
        return res
...
class HDLC(object):
    ...
    def sendFrame(self, data):
        ...
        logger.info("Sending Frame: %s", bs.hex())
    ...
    def _onFrame(self, frame):
        ...
        logger.info("Received Frame: %s", s.hex())
    ...
    def _onError(self, frame):
        self.last_frame = frame
        s = self.last_frame.toString()
        logger.warning("Frame Error: %s", s.hex())
    ...
    def _readBytes(self, size):
        cnt = 0
        while cnt < size:
            b = bytes(self.serial.read(1))
            if len(b) < 1:
                return False
            cnt += len(b)
            res = self._readByte(ord(b))
            if res:
                return True
